...
def email_sender(state: AgentState):
    print("Sending email")
    email_llm = ChatGroq(model="deepseek-r1-distill-llama-70b", temperature=0.1)
    email_message = [
    ...
    msg.attach(MIMEText(email_response.content, 'html'))
...
class Agent:
    ...
    def __init__(self):
        ...
        self._tools_llm = ChatGroq(model="deepseek-r1-distill-llama-70b").bind_tools(TOOLS)
